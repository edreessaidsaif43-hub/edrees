...
export async function joinTeacherMotivationClassByCode(payload) {
  ...
  try {
    ...
    `;
    const row = rows?.[0] || null;
    if (!row) return { error: "not_found", message: "Class code not found." };

    const classData = row?.class_data && typeof row.class_data === "object" ? row.class_data : null;
    if (!classData) return { error: "not_found", message: "Class data missing." };

    const teacherUserIds = new Set(
      Array.isArray(row?.teacher_user_ids) ? row.teacher_user_ids.map((x) => String(x || "").trim()).filter(Boolean) : []
    );
    ...
    await motivationSql`
      ...
      SET class_data = ${JSON.stringify(nextClassData)}::jsonb,
          teacher_user_ids = ${JSON.stringify(Array.from(teacherUserIds))}::jsonb,
          updated_at = NOW()
      WHERE shared_id = ${String(row.shared_id || "").trim()};
    `;

...
export default async function handler(req, res) {
  ...
  if (req.method === "GET" && action === "motivation_accounts") {
    const out = await listMotivationAccountsByAdmin();
    if (out.error === "db_not_configured") {
      res.status(500).json({
        error: "db_not_configured",
        message:
    ...
    if (out.error) {
      res.status(502).json({ error: out.error, message: out.message || "Failed to list motivation accounts." });
      return;
    }
    res.status(200).json({ accounts: out.data || [] });
    return;
  ...
  if (req.method === "GET" && action === "motivation_account") {
    const userId = String(req.query?.userId || "");
    if (!userId) {
      res.status(400).json({ error: "invalid_payload", message: "Missing userId." });
      return;
    }
    const out = await loadMotivationAccountByAdmin(userId);
    if (out.error === "db_not_configured") {
      res.status(500).json({
        error: "db_not_configured",
        message:
    ...
    if (out.error) {
      ...
      res.status(status).json({ error: out.error, message: out.message || "Failed to load motivation account." });
      return;
    }
    res.status(200).json(out.data);
    return;

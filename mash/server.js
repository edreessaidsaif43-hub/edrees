...
const OWNER_PASSWORD = process.env.MASH_OWNER_PASSWORD || 'owner12345';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/owner/login' && req.method === 'POST') {
      const body = await parseBody(req);
      if (String(body.password || '') !== OWNER_PASSWORD) {
        return send(res, 401, { error: 'كلمة مرور صاحب الموقع غير صحيحة' });
      }
      const token = crypto.randomBytes(24).toString('hex');

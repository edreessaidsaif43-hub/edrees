...
async function initDb() {
  ...
  await ensureColumn('projects', 'logo', 'TEXT');
  await ensureColumn('projects', 'media', 'TEXT');
  await ensureColumn('projects', 'links', 'TEXT');
...
async function listProjectsFor(user, scope) {
  if (scope === 'mine' && !user) {
    const rows = await all(`SELECT * FROM projects WHERE teacher_slug='demo-teacher' AND deleted=0 ORDER BY created_at DESC`);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if ((url.pathname === '/api/auth/login' || url.pathname === '/enjazy/api/auth/login') && req.method === 'POST') {
      const { contact, password } = await parseBody(req);
      const username = String(contact || '').trim().toLowerCase();
    ...
    if ((url.pathname === '/api/auth/register' || url.pathname === '/enjazy/api/auth/register') && req.method === 'POST') {
      ...
      if (body.action === 'update_profile') {
        ...
        if (password) {
          await run(`UPDATE users SET username=?, name=?, password_hash=? WHERE id=?`, [username, name, hashPassword(password), userId]);
        } else {
          await run(`UPDATE users SET username=?, name=? WHERE id=?`, [username, name, userId]);
        }
        return send(res, 200, { userId, profile: { name, contact: username } });
      }

      if (!password || password.length < 6) return send(res, 400, { error: 'invalid_payload' });
      const exists = await get(`SELECT id FROM users WHERE username=?`, [username]);
      if (exists) return send(res, 409, { error: 'user_exists' });
      const teacherSlug = username.replace(/[^a-z0-9_-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || `teacher-${Date.now()}`;
      const result = await run(
        `INSERT INTO users (username, password_hash, role, teacher_slug, name) VALUES (?, ?, 'teacher', ?, ?)`,
        [username, hashPassword(password), teacherSlug, name]
      );
      return send(res, 201, {
    ...
    if (url.pathname === '/api/register' && req.method === 'POST') {
      ...
      if (!username || !password || !name || password.length < 6) {
        return send(res, 400, { error: 'بيانات التسجيل غير مكتملة' });
      }

      const existing = await get(`SELECT id FROM users WHERE username=?`, [username]);
      if (existing) {
        return send(res, 409, { error: 'اسم المستخدم موجود مسبقاً' });
      }
      ...
      const token = crypto.randomBytes(24).toString('hex');
      await run(`INSERT OR REPLACE INTO sessions (token, user_id, expires_at) VALUES (?, ?, datetime('now', '+7 days'))`, [token, result.lastID]);
    ...
    if (url.pathname === '/api/projects' && req.method === 'POST') {
      ...
      const project = {
        ...
        logo: body.logo || '',
        media: JSON.stringify(Array.isArray(body.media) ? body.media : []),
      ...
      await run(
        `INSERT INTO projects (
          id,title,teacher,teacher_slug,school,category,subject,grade,description,views,featured,latest,weekly_top,
          public_in_main,admin_approved,hidden,deleted,cover,logo,media,links,problem,goals,steps,evidence,results,recommendations
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          ...
          project.cover, project.logo, project.media, project.links, project.problem, project.goals, project.steps, project.evidence, project.results, project.recommendations
        ]
      );
    ...
    if (url.pathname.startsWith('/api/projects/') && req.method === 'PATCH') {
      const user = await auth(req) || { role: 'admin' };

...
async function fileToMediaItem(file) {
  ...
  if (file.size > maxSize) throw new Error(`الملف ${file.name} أكبر من 15MB`);
  return {
    name: file.name,
...
function apiUrl(path) {
  if (path === '/api/projects') return '/api/mash/projects';
  if (path.startsWith('/api/projects?')) return path.replace('/api/projects', '/api/mash/projects');
  if (path.startsWith('/api/project/')) return `/api/mash/project?id=${encodeURIComponent(path.split('/').pop())}`;
  if (path.startsWith('/api/projects/')) return `/api/mash/project?id=${encodeURIComponent(path.split('/').pop())}`;
  if (path === '/api/admin/stats') return '/api/mash/stats';
  if (path === '/api/owner/login') return '/api/mash/owner-login';
  return path;
}
...
async function renderProjectDetail() {
  ...
  try {
    ...
    page.innerHTML = '<section class="section"><div class="container panel">تعذر تحميل المشروع من قاعدة البيانات. تأكد من إعداد DATABASE_URL وتشغيل الخادم.</div></section>';
  }
}
...
async function submitProject(ev) {
  ...
  const logo = logoFile ? await readFileAsDataUrl(logoFile) : '';
  const media = [];
...
async function loadHome() {
  ...
  if (loadError) {
    ...
    if (featured) featured.innerHTML = '<p>تعذر تحميل المشاريع من قاعدة البيانات. تأكد من إعداد DATABASE_URL وتشغيل الخادم.</p>';
  }

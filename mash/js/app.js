...
function localProjects() {
  return Array.isArray(window.LOCAL_PROJECTS) ? window.LOCAL_PROJECTS : [];
}
...
async function renderProjectDetail() {
  ...
  try {
    ...
    const p = localProjects().find(project => project.id === id);
    if (p) {
      renderProjectPage(page, p);
      return;
    }
...
async function loadAdminData() {
  const stats = await api('/api/admin/stats', { headers: ownerHeaders() });
...
async function loadHome() {
  try {
    ...
    try {
      state.projects = await fetch('data/projects.json').then(res => res.json());
    } catch (fallbackError) {

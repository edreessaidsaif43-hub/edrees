...
function updateSectionCardToggleLabels() {
  const cards = Array.from(document.querySelectorAll(".section-card"));
  cards.forEach((card) => {
    const btn = card.querySelector(".card-guide-toggle");
    if (!btn) return;
    btn.textContent = card.classList.contains("mobile-open") ? "إخفاء التفاصيل" : "عرض التفاصيل";
  });
}

function initSectionCardsMobile() {
  const cards = Array.from(document.querySelectorAll(".section-card"));
  cards.forEach((card) => {
    const guide = card.querySelector(".card-guide");
    if (!guide) return;
    let btn = guide.querySelector(".card-guide-toggle");
    if (!btn) {
      btn = document.createElement("button");
      btn.type = "button";
      btn.className = "card-guide-toggle";
    ...
    btn.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (!isMobileGuideMode()) return;
      const willOpen = !card.classList.contains("mobile-open");
      cards.forEach((c) => c.classList.remove("mobile-open"));
      if (willOpen) card.classList.add("mobile-open");
      updateSectionCardToggleLabels();
    });
  });
  if (!isMobileGuideMode()) {
    cards.forEach((c) => c.classList.remove("mobile-open"));
  }
  updateSectionCardToggleLabels();
}

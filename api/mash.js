      ...
      if (req.method === "DELETE") {
        if (!ownerTokenValid(req)) {
          ...
          if (!ownerMatchesById && !ownerMatchesBySlug && !ownerMatchesByIdentity) {
            return send(res, 403, { error: "owner_only", message: "You can only delete your own projects." });
          }
        }
        const out = await updateMashProject(id, {
          deleted: true,
          hidden: true,
          publicInMain: false,
          latest: false,
        });
        if (out.error) return sendStoreError(res, out, "Could not delete project.");
